...
function UploadArea({ setPage, setMetadata }) {
	...
	let icon = <IoCloudUploadOutline className="h-8 w-8 group-hover:h-10 group-hover:w-10 duration-300" />;
	...
	return (
		...
		>
			<div className="flex h-30 w-30 items-center justify-center pointer-events-none">
				...
				/>
				<div className={`flex items-center justify-center h-20 w-20 rounded-full group-hover:scale-110 transition-transform duration-300 ${iconColors}`}>
					{icon}
				</div>
			</div>
			
			<div className="flex flex-col gap-1 items-center pointer-events-none">
				<div className={`font-bold text-lg ${error ? "text-red-600" : "text-slate-600"}`}>
					{message}
				</div>
				<div className={`text-sm font-medium ${error ? "text-red-400" : "text-slate-400"}`}>
					{subMessage}
				</div>
			</div>
